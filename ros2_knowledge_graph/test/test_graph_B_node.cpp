...
public:
  NodeB()
  : rclcpp::Node("node_B"), gen_(rd_()), dis_(1, 10)
  {
    graph_ = std::make_shared<ros2_knowledge_graph::GraphNode>("node_B");
    graph_->start();
  }
  ...
  {
    {
      ...
      if (new_value >= 3 && new_value <= 5) {
        ...
        ros2_knowledge_graph::Node node{name, type};
        ...
        ros2_knowledge_graph::Edge edge{content, type, source, target};
